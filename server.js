...
app.post("/api/jobs/:id/pay", async (req, res) => {
  if (!stripe) return res.status(500).json({ error: "Stripe is not configured on this
  await db.read();
});
server"
...
app.post("/api/stripe/webhook", express.raw({ type: "application/json" }), async (req,
...
app.listen(PORT, () => {
  ...
  if (!stripe) console.warn("STRIPE_SECRET_KEY not set — payment endpoints will fail u
});